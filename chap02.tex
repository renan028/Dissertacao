...
\begin{figure}[!ht]
\centering
\includegraphics[width=.5\columnwidth]{figs/SHAKEY_1.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=.5\columnwidth]{figs/HILARE.jpg}
\caption{HILARE}
\label{hilare}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=.5\columnwidth]{figs/MANIPULADOR.jpg}
...
Executivo e Funcional.

O planejador é a uma camada composta pelo planejador de missão e o
cartógrafo, gerando objetivos e planos estratégicos. Esses objetivos são
repassados para a camada intermediária, chamada Executivo. 

A camada Executivo utiliza técnicas de planejamento reativo para selecionar um
conjunto de comportamentos de uma biblioteca, e desenvolve uma rede de tarefa
especificando a sequência de execução para os comportamentos a um subobjetivo
particular. De acordo com Murphy, o Executor seria a camada responsável por
sequenciar e monitorar o desempenho de uma arquitetura híbrida.

Os comportamentos selecionados para uma tarefa pelo Executor formam a camada
Funcional. A tabela~\ref{3Ttable} resume os componentes desta arquitetura
híbrida.
...
Em \cite{staranowicz2011survey} e \cite{kramer2007development} foi realizada uma
compilação de RDEs, apontando suas vantagens, desvantagens e aplicações. Neste
trabalho, são destacados os dois sistemas abertos mais populares: Player e ROS,
e, por fim, é justificado a utilização de ROS para ser a RDE do robô DORIS.
...
programação de dispositivos (drivers) e sensores, em vez de robôs. Os
...
conexão com o servidor para transferência de dados, preservando operações
concorrentes. Múltiplos clientes podem subscrever a um mesmo dispositivo e podem
eviar comandos simultaneamente, mas não há fila e comandos antigos são
...
em qualquer linguagem de programação com suporte a soquete, por exemplo Python,
C++, Java e outros. Clientes podem operar em qualquer host que possua conexão,
possibilitando independência de localização.
...
repositório disponível no website ROS. O sistem se comporta de maneira
semelhante ao Player em relação à conexão por soquete TCP/IP, possibilitando o
...
agendamento, mas apresenta algumas ferramentas parecidas as de um SO como
mensagens entre processos. O sistema é baseado em nós, mensagens, tópicos e
serviços. 
...
comunicar com outros nós por simples mensagens, podem publicar e subescrever a
um ou múltiplos tópicos.
