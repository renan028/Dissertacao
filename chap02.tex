...
necessidade do desenvolvimento de programas abertos para simulação e interface
em robótica.
...
programas que visam ajudar na implementação de cada camada. Em seguida, são
abordados os programas e linguagens para cada camada, analisando os
...
\cite{elfes1983distributed} idealizou uma arquitetura em módulos e controle
...
escolha por ROS como o RDE do sistema robótico DORIS.
...
hardware abstraction layer), controle baixo nível de dispositivos, mensagens
entre processos TCP/IP ou UDP, e gerenciamento de pacotes. Dentre todas, a mais
importante característica do ROS é a presença de uma grande comunidade de
pesquisadores que contribui para a expansão do software, que é possível, principalmente, pela
...
diferentes \textit{hosts}, conectados por uma topologia ponto-a-ponto, em tempo
de execução. Uma topologia com servidor central começa a se tornar problemática se
...
\textit{socket} TCP/IP, possibilitando o uso de múltiplas
...
A ideologia \textit{thin} de ROS é possibilitar que códigos, como
\textit{drivers} e algoritmos, possam ser reutilizados em outros
\textit{middlewares}, fora de ROS.
...
Para trocas de mensagens síncronas em ROS, utiliza-se \textit{service} (também
...
também rodando nós de ROS. Os componentes no robô são \textit{drivers} que se
comunicam com os diversos hardwares do robô (sensores e atuadores), algoritmos para
...
As vantagens da utilização de ROS no desenvolvimento de sistemas robóticos são
...
vantagens, a arquitetura reativa possui problemas de contextualização e
...
durante todo o funcionamento do robô. O problema de contextualização é a
...
Outro problema da arquitetura de subsunção é o \textbf{conhecimento da falha}
...
Todas as linguagens mencionadas fornecem suporte à decomposição hierárquica de
...
PNs ainda requer software externo independente, JARP, para a modelagem das PNT.
 
\subsubsection{SMACH}\label{smach}

Em 2011, Bohren testou SMACH, camada Executivo, no robô PR2, integrado
...
deixa o sistema muito mais flexível. Ela permite que dados sejam passados entre
...
de pesquisa no campo de inovação de tecnologia de software e robótica baseado em métodos de inteligência artificial, desenvolveu um robô
...
planning}), mapeamento, e navegação (Figura~\ref{rapyuta}).
...
\begin{figure}[!ht]
\centering
\includegraphics[width=1\columnwidth]{figs/rapyuta.png}
\caption{Compartilhamento de conhecimento entre robôs, a internet dos robôs -
Rapyuta.}
\label{rapyuta}
\end{figure}