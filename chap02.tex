...
comportamentos de baixo nível, executando-os apropriadamente. Na arquitetura
...
máquina de estados ou Redes de Petri \cite{peterson1981petri}. Algumas
...
Comparando-se  com sistemas baseados em máquina de estados (FSA), as PNs são
...
ao sistema ROS~\ref{bohren2011towards}. PR2 é um robô planejado a executar uma
...
\cite{harel1987statecharts}). As características do sistema são detalhados a
seguir, já que a implementação da arquitetura do robô DORIS também utiliza SMACH
como parte de sua camada Executivo:

1) Estruturas: há duas interfaces primárias definidas por SMACH:
...
\begin{itemize}
  ...
  única função \textit{execute}, que bloqueia até retornar um \textit{outcome}
  para aquele \textit{state}. \textit{Outcomes} de \textit{states} podem ser
  pensados como uma ``interface'' para um dado \textit{state}.
