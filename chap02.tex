...
desenvolvimento de novas arquiteturas são o âmago no estudo de autonomia e
...
\section{Conceitos base}\label{conceitos}

Faz-se necessário a definição de alguns conceitos base para o entendimento desta
...
para o robô DORIS, sendo considerados os aspectos físicos, lógicos e a
aplicabilidade do robô, entende-se que:

\textbf{Definição:} Uma \textbf{arquitetura robótica} descreve
...
\textbf{Definição:} o controle de missão de robôs é o componente da arquitetura
...
ser mais simplificada, já que este se move ao longo de um trilho.
...
\subsection{Aquitetura híbrida}\label{hibrida}
Em \cite{arkin1998behavior}, Arkin aponta quatro possíveis estratégias para o
projeto de arquiteturas híbridas:

\begin{itemize}
  \item Seleção: a deliberação é vista como um configurador. O planejador
  ...
  no sistema.
  \item Conselho: a deliberação é vista como um aconselhador. O planejador
  sugere mudanças que o controle reativo pode ou não usar.
  \item Adaptação: a deliberação é vista como um adaptador. O planejador
  ...
  \item Adiamento: a deliberação é vista como um recurso para ser usado em
  último caso. Neste caso, os planos só são elaborados quando necessários.
\end{itemize}
...
escolha por ROS como o RDE do robô DORIS.
...
processamento (imagens e outros), e controladores dos atuadores robô. Na base,
componentes mostram os status do robô para o usuário e exercem a função de
visualização. Está sendo representada tanto a comunicação tipo
\textit{service} (servidor-cliente), quanto a comunicação broadcast.
...
entre si numa rede \textit{peer-to-peer}. Há as duas formas de
comunicação entre os módulos: o estilo síncrono baseado servidor-cliente e o
assíncrono \textit{publisher-subscriber}. Alem disso, ROS suporta simuladores
robóticos como Stage, Gazebo, MORSE e OpenRAVE.

\subsection{Camada Executivo}

Como já definido na subseção~\ref{3t}, em uma linguagem ROS, a camada Executiva
usa técnicas de planejamento reativo para selecionar os nós, ou componentes, que
entrarão em execução de uma biblioteca de componentes existentes. Ela sequencia
a execução dos componentes para a realização de um certo objetivo, monitora o
desempenho, traduz os planos de alto nível da camada Planejador em
tarefas, executando-os apropriadamente. Na arquitetura
implementada, a camada Executiva também aloca e monitora os recursos
...
escala, os quais a camada Executiva deve se preocupar em resolver.

O problema de escala ocorre devido ao número de interconexões de uma arquitetura
reativa, o qual aumenta exponencialmente em relação ao aumento de coportamentos.
Isto ocorre, pois um comportamento obtem os dados de outros
comportamentos através de interconexões (um comportamento de nível 1 necessita
estar interconectado a comportamentos de nível 0 para obter os seus dados). Este
problema já havia sido resolvido por uma arquitetura com comunicação do tipo
\textit{blackboard} \cite{elfes1983distributed}, onde os dados dos sensores são
...
camada Executiva deve reportar a falha à camada Planejador (\textit{execution
...
síncrono do mundo. O modo de monitoramento de tarefas é específico em cada
linguagem e será analisado a subseção~\ref{monitoamento}.

As linguagens lidam de maneira diferentes em relação a monitoramento de execução
e recuperação de erros. ESL, TDL, SMACH e PNPs provêem monitoramento explícito
de tarefas, verificam erros com registros, como feito em C++ e Java, e suportam
procedimentos de ``limpeza'', quando as tarefas são terminadas. RAPs e PLEXIL
...
provêem a noção de recursos compoartilhados e gerenciamento de recursos.

Detalha-se, em seguida, a camada Executivo integradas ao sistema ROS.
...
Comparando-se  com sistemas baseados em máquinas de estados (FSA), as PNs são
geralmente exponencialmente mais compactas, já que, por exemplo, uma PN finita
pode representar uma FSA infinita.
...
que exijam robustez. Além disso, a interface com ROS ainda está em
desenvolvimento, ainda não é possível executar PNs em paralelo, e a construção
das PNs requerem software externo independente (JARP).
...
Componentes de planejamento incluem: navegação, planejamento movimentos
...
da arquitetura do robô DORIS também utiliza SMACH como parte de sua camada
Executivo.
...
tipo \textit{split-join} (dividir-juntar): \textit{Concurrence} (Concorrente).
...
Os \textit{outcomes} de um \textit{Concurrence} podem ser definidas em sua
...
\caption{Container \textit{SM_TOP} que possui dois estados e transições de
dados.}
\label{container}
\end{figure}
...
\begin{itemize}
  ...
  robô podem ser divididos para múltiplos programados sem conflito.
  \item SMACH pode ser executado a partir do meio de um plano sem alteração no
  código.
...
camada Executivo completa, pois esta camada possui outras responsabilidades
...
camada de nível Executivo. E já há sistemas que se baseiam na filosofia SMACH, como em
\cite{wachsmuth2012tobi}.

\subsection{Camada Planejador}

Na subseção~\ref{3t}, o Planejador é definido como uma camada composta pelo
planejador de missão e o cartógrafo (modelo do mundo). A camada determina as
...
A duas abordagens mais comuns são o planejador de rede de tarefas hierárquicas
(\textit{hierarchical task net} - HTN) e o planejador/agendador. Os HTN, como o
...
decompõem tarefas em subtarefas de uma maneira similar às camadas Executivas. A
diferença é que os HTN operam em um alto nível de abstração, tomam em
conta a utilização de recursos, e possuem métodos de lidar com conflitos entre
tarefas (tarefas que necessitam o mesmo recurso, ou uma tarefa negando uma
pré-condição necessária por outra tarefa). A base de conhecimento do robô é o
...
determinar exatamente como alcançar as objetivos. Planejador/agendador organizam
...
robô inclue os objetivos das tarefas, os recursos necessários, suas durações, e
restrições entre tarefas.

\subssubection{Rapyuta}
...
prover aplicações por meio da internet projetado especificamente para
aplicações robóticas. A grande motivação é o número cada vez maior de robôs
saindo do ambiente industrial para o ambiente doméstico, praticando tarefas
caseiras como servindo bebidas, dobrando toalhas e outras. Algumas tarefas
...
ser realizadas na nuvem, como planejamento de segurar objetos (\extit{gasp
