...
determinado objetivo, e o conjunto de tarefas ordenado sequencialmente compõe a
missão. Uma tarefa pode ser cancelada por outras tarefas (subsunção). Em
linguagem de arquitetura robótica, esta definição se confunde com \textbf{comportamentos} por
ser um processo básico e ser modelado como uma AFSM, porém é uma definição mais
geral por não necessariamente envolver um par estímulo-reação/sensor-atuador. \textbf{Tarefa} é o processo básico definido na
camada Executivo SMACH.
...
O componente \emph{IMU} é um driver que faz a interface com a \emph{IMU}.
...
comportamento reativo é uma máquina de estados aumentada (\emph{AFSM}), como a
...
\begin{itemize}
  \item \textbf{STATE\_OF\_CHARGE}: verifica a energia disponível do robô
  (nível das baterias). Caso esta esteja inferior a $5\%$, o robô deve parar
  todos as missões e processos reativos.
  \item \textbf{STATE\_OF\_TEMPERATURE\_AND\_HUMIDITY}: verifica condições de
  temperatura e umidade externas. Quando essas estão fora do nível de operação,
  o robô suspende as missões.
\end{itemize}
...
\begin{itemize}
  \item \textbf{OBSTACLE\_DETECTION}: detecta a presença de obstáculos por
  câmera fixa (algoritmo de anomalias no trilho) ou pelo sensor de consumo de
  corrente dos motores, que, ao ficar muito elevado, representa choque com algum
  obstáculo \cite{christensen2011cost}.
\end{itemize}

Os \textbf{processos reativos vitais} e os \textbf{processos reativos
...
subsunção. Desta forma, foi implementada uma classe para a AFSM padrão de todas
as tarefas da DORIS, implementada em SMACH:


A camada Executivo é a camada, em DORIS, que assumirá
