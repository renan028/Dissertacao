...
camadas, como planejamento global e planejamento local. No aiatema robótico
DORIS, não há um planejamento de trajetória, já que o trilho é um mapa unidimensional, porém há um planejamento de velocidades
...
\begin{defn}
\textbf{Estado} é o processo básico definido na
...
interface com o usuário, o acompanhamento e \textit{feedback} das missões ao
usuário e a tradução das missões ao robô. Portanto, o controle de missão é a parte da
arquitetura robótica que faz a interface Homem-Robô: traduz a linguagem do
homem à linguagem do robô e vice-versa.

A interface de usuário do sistema robótico DORIS possui o conjunto de missões e
seus parâmetros disponíveis para o operador. O usuário pode adicionar missões sequencialmente,
salvar um plano de missão, agendá-lo e/ou executá-lo (botão \textit{Play}). Além
disso, a interface disponibiliza os \textit{logs} das missões.
...
que serve cafés. A implementação desta responsabilidade no sistema robótico
DORIS é realizada por um dicionário codificado no robô.
...
\begin{defn}
\textbf{Missões simples} não necessitam de tradução por já estarem modeladas em
tarefas e máquinas de estados SMACH. 
\end{defn}
...
\begin{itemize}
  ...
  absoluto; velocidade desejada: ``slow'' (devagar), ``normal''
...
\begin{defn}
\textbf{Missões complexas}, de mais alto nível, são combinações de
...
\begin{defn}
\textbf{Missões desconhecidas} são missões que não estão
descritas no robô, isto é, missões que, até o momento, não pertencem ao
dicionário do robô. 
\end{defn}

Até esta fase da implementação, o sistema robótico DORIS não
permite que o usuário use missões desconhecidas. O motivo principal disso é que o robô não
...
O controle de missão está implementado, mas ainda não foi embarcado no robô. A
interface gráfica de usuário ainda não foi finalizada, mas esta pode ser
simulada por mensagens de ROS, durante os testes.
...
Com o Agendador, o robô ganha uma rotina de missões, o que é importante ao
sistema robótico DORIS, um robô de inspeção.
...
robô. No caso do sistema robótico DORIS, por exemplo, durante o planejamento de
velocidades do robô, o mapa geométrico 3D do ambiente não é a melhor opção, já
que o sistema robótico DORIS se movimenta em um espaço unidimensional (o
trilho), e carregar e um ambiente 3D exige muito tempo de
processamento de uma CPU. A camada Planejador deve gerar um mapa unidimensional, ou um mapa simbólico, para otimizar a função de planejamento de
velocidades. Além disso, a camada Planejador deve manter os mapas gerados
atualizados.

O sistema robótico DORIS possui diversos mapas do mundo:
\begin{enumerate}
...
Os mapas utilizados para a aplicação de inspeção são 1, 2, 3 e 6, e foram
gerados com os sensores câmera fixa, câmera termográfica, microfones e laser, respectivamente.

O mapa 9 é obtido durante a fabricação do trilho e não é mais alterado. 

O mapa 4 foi gerado pelo CAD da fabricação do trilho, ou seja, a partir do mapa
9, e será necessário para a locomoção do robô. 

Os mapas 5 e 8 são gerados a partir do mapa 6 e são necessários para a
localização do robô. 

O mapa 7 é inserido manualmente pelo programador, sendo uma matriz que relaciona o equipamento às
...
A camada funcional foi a primeira implementada e exaustivamente testada no
sistema robótico DORIS. Como nas outras arquiteturas de três camadas previamente
apresentadas, a camada Funcional é o nível mais baixo, conecta sensores a atuadores, implementa controladores PID, e diversos outros algoritmos, por
...
sistema, a camada funcional é uma interface com todo o sistema de hardware e
suas capacidades, é um software orientado a objeto, obtendo assim modularidade
de hardware, e uma estruturação apropriada para usar as propriedades de
...
No sistema robótico DORIS, foi proposto o \textit{Robot Package Software}. Neste
sistema, \emph{Tools} (janelas gráficas) e \emph{Componentes} (unidades de comunicação e
processamento) são agrupados em \textit{Robot Package} (bibliotecas dinâmicas).
Os componentes que lidam com hardware são executados no computador embarcado do
robô, já os componentes que interagem com as \textit{tools} são executadas no
computador base.
...
Dois \textit{Robot Packages} fazem parte do sistema robótico DORIS:
...
O \textit{DORIS Package} é um pacote mais específico ao sistema robótico DORIS e
lida com seus elementos de hardware e suas funcionalidades. Do ponto de vista de uma
arquitetura robótica, neste pacote foram implementados os \emph{comportamentos}
específicos do sistema.  Os diversos componentes implementados para o sistema
robótico DORIS foram divididos em funcionalidades, neste trabalho, a fim de
...
(driver) utilizado no sistema robótico DORIS, além de implementar outras
caracterísiticas do driver, como os dados dos encoders dos motores: posição (odometria), velocidade
e corrente.
...
O manipulador do sistema robótico DORIS utiliza mais quatro EPOS (quatro
motores) e exigirá ainda a implementação de um controle de força para sensoriamento de equipamentos ao
...
O sistema robótico DORIS se locomove em um trilho (mapa unidimensional), logo
não há a necessidade de computação de um \textit{Motion Planning} complexo. Quando uma
...
autônomo, o robô será controlada por posição, havendo a necessidade de
...
A camada Executivo elaborada para o sistema robótico DORIS usa SMACH
(Subseção~\ref{smach}) como \textit{framework}, e implementa a técnica de função
de coordenação competitiva (Subseção~\ref{reativa}), a arquitetura de subsunção,
onde o módulo básico de comportamento reativo é uma máquina de estados aumentada (\textit{AFSM}), como a
...
\begin{defn}
\textbf{Processo reativo vital} é um processo que deve ser
...
\begin{defn}
\textbf{Processo reativo específico} é um processo
...
O problema de contextualização (Subseção~\ref{brooks}) é resolvido pela
...
\paragraph{Processos reativos vitais no sistema robótico DORIS}

Os \textbf{processos reativos vitais} criados para o sistema robótico DORIS são:
\begin{itemize}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/camera.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/epos.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/mainoa.png}
...
\paragraph{Tarefas no sistema robótico DORIS}
As tarefas criadas para o sistema robótico DORIS são detalhadas sequencialmente,
conforme a missão simples:
\begin{itemize}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/main_goto.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/mainlocalization.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/mainmotioncontrol.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figs/mainrecord.png}
...
manipulador do sistema robótico DORIS não tem condições de inspecionar dois
equipamentos ao mesmo tempo; um robô com um manipulador que serve bebidas não é
capaz de servir três bebidas simultaneamente; um robô submarino com apenas uma
câmera sob um sistema \textit{Pan \& Tilt} (atuador) não pode compartilhar a câmera para navegação e
...
\begin{defn}
\textbf{Processo reativo de recurso disponível} é um
...
obtendo sempre uma falha ``misteriosa''. Para um programador seria ainda
necessário o debug do código completamente, e mesmo somente nas tarefas que contém a missão
...
recuperação, a qual busca uma nova trajetória para o sistema robótico DORIS
