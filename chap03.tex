...
\textbf{Definição}: \textbf{Tarefa} é o processo básico definido na
camada Executivo SMACH. A tarefa é um processo executado pelo robô com um
determinado objetivo e seu conjunto ordenado sequencialmente completam a missão.
Uma tarefa pode ser cancelada por outras tarefas (subsunção). Em linguagem de
arquitetura robótica, esta definição se confunde com \textbf{comportamentos} por
ser um processo básico e ser modelado como uma AFSM, porém é uma definição mais
geral por não necessariamente envolver um par estímulo-reação/sensor-atuador.
...
figura~\ref{afsm} sem \textit{Reset}. A camada Executivo é o foco da
implementação do autor desta dissertação e, junto com a camada Planejador,
permite a autonomia do robô.
...
ao robô, garante robustez, proteção e a sobrevivência do sistema e aos seus dispositivos.

Dependendo da missão a ser realizada, além das tarefas que a especificam, um
conjunto de processos paralelos começa a ser executado de maneira automática, às vezes inconscientemente, a fim de
...
automáticos e executados somente durante a realização da missão. 
...
inicializados automaticamente de maneira insconsciente. Podem garantir
