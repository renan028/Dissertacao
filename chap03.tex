...
\emph{arquitetura robótica} não é equivalente a uma arquitetura de
...
contexto mais amplo de arquitetura robótica. A arquitetura ideal para um robô
pode ser ineficiente para outro. A arquitetura robótica desenvolvida tenta
ser geral para robôs com características e desafios semelhantes ao DORIS, como
AUVs e UGVs.
...
\begin{itemize}
  ...
  equipamentos); $\textbf{MANIPULATOR\_POSITION\_CONTROL([x,y,z])}$: posição
...
Com o Agendador, o robô ganha uma rotina, o que é importante a DORIS, um robô de inspeção. A responsabilidade Agendador, porém, ainda não foi
implementada.
...
autonomia do robô, mas apenas a uma fração do controle de missão, que diz respeito à interface com o usuário e \textit{feedback} dos
planos ao usuário.
...
velocidades diferentes. O \emph{MotionController} envia \textit{set-points} de
...
responsabilidades de uma camada Executivo. A camada Executiva não executa
algoritmos de alto processamento, como a camada Funcional, logo pode ser
escolhida a linguagem Python para programação.
...
os processos e escolher qual entrará em ação é chamada de \textbf{Selecionador},
...
cancela-os quando são abortas ou finalizadas.
...
\textbf{Definição}: \textbf{Processos de recursos disponíveis} são processos
reativos que monitoram os status dos recursos do robô (sensores e atuadores).
Estes processos podem abortar missões, por exemplo, uma falha no
\textbf{processo de recurso disponível} \textit{EPOS\_RESOURCE} aborta a missão
\textbf{GOTO} caso esta esteja no plano de missão do robô, pois não há como o
robô se locomover sem o driver da EPOS.

Os \textbf{processos de recursos disponíveis} são como os \textbf{processos
reativos vitais}, ou seja, estão sempre em execução, desde a inicialização do
