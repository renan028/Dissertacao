...
\begin{itemize}
  ...
  sensores e atuadores físicos; e o desempenho computacional. Este é exclusivo
  da camada Funcional.
...
sistema autônomo. O planejamento de velocidades é apenas um algoritmo cuja saída
...
Nesta dissertação, é documentado um exemplo do planejamento de velocidades.
...
A avaliação da camada Executivo é a análise e testes por simulação das
responsabilidades dentro dos critérios de Arkin. Isto é, como o modo de
implementação de cada responsabilidade da camada se comporta em face dos
...
tarefa \textbf{PathPlanning} requisita o plano ao algoritmo
\textbf{MasterPlanning} da camada Funcional, e após recebido a missão
\textbf{GOTO} segue para sua última tarefa \textbf{Motion\_Control\_Position}.
...
baixo nível. Ao receber a mensagem [30,1] (posição 30 e probabilidade 1), a tarefa é
completada, e a tarefa seguinte, \textbf{PathPlanning}, recebe o dado de posição
por \textit{user data} SMACH. Esta envia por mensagem de ROS comandos ao
\textbf{MasterPlanning}, recebe o plano deste e o envia a
\textbf{Motion\_Control\_Position} também por \textit{user data} SMACH. A
figura~\ref{transgoto} mostra as transições das tarefas descritas pelo terminal
do Ubuntu onde: em azul está o recebimento da localização e dados da missão (30
é posição atual, 80 é posição objetivo, 'fast' é o modo de velocidade rápida);
em verde é a matriz de velocidades e pontos de troca gerada pelo
...
vermelho, é o loop da tarefa \textbf{Motion\_Control\_Position}, a qual recebe
...
missão por processo reativo de recurso disponível; e interrupção de missão por
processo reativo específico. 
...
(terminal) e figura~ref\ref{abortgoto} (smach\_viewer, tarefa em cinza
...
recebe a mensagem ``False'' no tópico \textit{'DORIS/MCS/StateOfCharge/Status'}
...
trajetório (sentido contrário).

Dessa forma, as responsabilidades \textbf{Sequenciador} e \textbf{Selecionador}
da camada Executivo atendem aoss critérios \textbf{Suporte a paralelismo},
...
paralelos, o mecanismo mais comum encontrado na natureza. Portanto, a camada
Executivo passa por todos os critérios de Arkin.
...
Arkin, ou seja, é a análise dos \textbf{controle de missão}, \textbf{agendador}
e \textbf{cartógrafo} em face aos critérios.
...
Portanto, o \textbf{controle de missão} traduz as missões complexas, isto é,
...
executado, junto com as missões simples que compõe as missão, de maneira
paralela ou sequencial. A organização cria uma ferramenta para implementação de
missões, agilizando o \textbf{Tempo de desenvolvimento}. Além disso, a
flexibilidade na modelagem das missões complexas e o paralelismo permitem a
...
dos \textit{outcomes}, e aos \textbf{Agendador} e \textbf{controle de missão},
na camada Planejador. Erros nas missões programadas são agendadas para nova
execução, no futuro, porém esta responsabilidade ainda não foi implementada na
camada Planejador, logo não será analisada nos resultados. O feedback
ao usuário disponível pelo \textbf{controle de missão}, que se comunica à camada
Executivo, também é uma forma de \textbf{Robustez}, pois é uma informção que
pode ser interpretada e utilizada para algumas tomadas de decisão. 
...
diversidade do \textbf{cartógrafo}, o qual pode gerar diversos modelos de mundo,
dependendo da aplicação, comunicando-se com os algoritmos da camada Executivo.
...
da missão complexa; execução da missão; feedback ao usuário. Como a interface de
usuário ainda não está finalizada, as mensagend de usuário podem ser
perfeitamente simuladas via terminal.
...
\begin{itemize}
  ...
  (missão complexa) só prossegue após a finalização da missão simples em
  ``join()'', o que mostra o caráter sequencial. As missões simples sequenciais
...
\begin{figure}[!ht]
\centering
\includegraphics[width=.8\columnwidth]{figs/executeinspection.png}
