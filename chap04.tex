...
As camadas desenvolvidas para o sistema robótico DORIS foram projetadas para
passar satisfatoriamente nos critérios de Arkin e de outros roboticistas. Dessa forma,
...
no robô. Como a camada Funcional é a camada que faz interface com os
elementos de hardware do robô (sensores e atuadores), simulações não bastam para
a avaliação dos critérios de Arkin, logo são necessários testes exaustivos, em campo, com o
robô.
...
A camada Funcional desenvolvida para o sistema robótico DORIS é detalhada na
...
robô, pacotes específicos do robô e pacotes com funcionalidades ROS.
...
teleoperação do SAM (\textit{Single Autonomous System}), predecessor do sistema
robótico DORIS, em uma interface web. Na figura, pode-se observar a imagem
enviada pela câmera interna do robô e o ambiente 3D em que o robô está inserido, em RVIZ. Na
Figura~\ref{robotgui}, mostram-se a interface de controle do sistema robótico
DORIS com dados de corrente e velocidade dos motores, a saída de vídeo da câmera interna ao robô,
...
variedade de aplicações robóticas. Além disso, os resultados do sistema robótico
DORIS mostram que o código implementado pode ser estendido a robôs com desafios semelhantes.
...
\item \textbf{Flexibilidade em tempo de execução}: os resultados do sistema
robótico DORIS com a teleoperação mostrou que pode ser controlada com alteração
dos parâmetros de controle em tempo real. O sistema de comunicação de ROS permite a
...
sucedidos mostraram o alto desempenho do sistema robótico DORIS. 
\end{itemize}
...
de Arkin, a camada não transforma o sistema robóticoDORIS em um sistema
autônomo.
As outras camadas, além de garantirem esta nova configuração do sistema, devem também
...
A função $master\_planning(0,'fast',10)$ retorna a tabela~\ref{matrizvmax}, que
...
e missões simples do sistema robótico DORIS, utilizando a classe implementada
...
\textbf{processos reativos} e as missões do sistema robótico DORIS já foram
documentados na subseção~\ref{impexecutivo} e ~\ref{missioncontrol}, respectivamente.
...
estado de supressão).
 ...
 \textbf{WANDER}, o qual controla o robô com velocidade $0.1 m/s$ até a certeza
 de posição aumentar para 60\%; quando \textbf{Motion\_Control\_Position} recebe dados de posição com certeza inferior
